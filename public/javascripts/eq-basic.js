$(document).ready(function() {
	  ...
	  var equalizer = context.createBiquadFilter();
	  ...
	  function getData() {
	  	...
	  	$.getJSON("../../data/eq/" + url + ".json", function(data) {
		   exerciseData = data;
		   createExercise(); 
		});	 
	  }
	  ...
	  function createExercise() {
		 ...
		 // Load sounds
		 audio.src = "../../sounds/eq/" + exerciseData[drillNumber].sounds;
	  ...
	  function playChanged() {
		 ...
		 equalizer.gain.value = 12;
		 if (!changeActive) {
		    randomNr = Math.floor(Math.random() * exerciseData[drillNumber].nrOfButtons);	
		 }
		 equalizer.frequency.value = exerciseData[drillNumber].frequencies[randomNr];
		 changeActive = true;
		 console.log(equalizer.frequency.value);
		 audio.play();   
	  ...
	  function updateUserStats() {
		  ...
		  var progress = (($(".progress-bar").width() / 10) * (questionCount));
	      
		  //$('.progress-bar-progress').width(progress * (questionCount - 1));
